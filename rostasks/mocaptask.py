...
def processed_multiple_bag(bagNames, outBagName, system_id, mocap_topic, groundtruth_odom_topic, groundtruth_path_topic):
    with rosbag.Bag(outBagName, 'w') as outBag:
        ...
        for bagName in bagNames:
            ...
            count = 0
            for topic, msg, t in current_bag.read_messages(topics=[mocap_topic]):
...
class MocapTask(Rostask):
    ...
    def execute(self):
        ...
        processed_multiple_bag(self.bags, output_bag, self.system_id,
                               self.mocap_topic, groundtruth_odom_topic, groundtruth_path_topic)
