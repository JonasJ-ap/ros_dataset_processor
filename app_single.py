...
def get_run_name(properties):
    if name := properties.get("single_run_name"):
        return name
    raise ValueError("single_run_name not set in properties")


def main(config_file):
    properties = get_properties(config_file)
    rosproject = Project(properties=properties)
    rosproject.processRun(get_run_name(properties), get_tasks(properties))
